...
const exerciseIcon = key => ({ squat: 'activity', pushup: 'move-down', lunge: 'footprints', plank: 'minus', deadlift: 'dumbbell', bicepCurl: 'armchair' }[key] || 'dumbbell');
...
function startSet() {
  ...
  const ssb = $('startSetBtn');
  if (ssb) ssb.disabled = true;

  if (ex?.isTimerExercise) {
    // ── PLANK / TIMER MODE ──────────────────────────────────────────────
    state.plankElapsed  = 0;
    state.plankDuration = parseInt($('targetRepsSelect')?.value) || ex.defaultDuration || 30;

    const repEl = $('repCounter');
    if (repEl) repEl.textContent = `${state.plankDuration}s remaining`;
    ...
    toast(`⏱️ Hold for ${state.plankDuration} seconds!`, 'success');
    aiSay(`Timer started! Hold your plank for ${state.plankDuration} seconds. Keep that core tight and breathe steadily `);
    ...
    aiSay(`Ready. Go for your ${state.targetSetReps} ${ex?.name} reps. I am watching your form.`);
  }
}
...
function selectExercise(key) {
  ...
  // Swap target selector: timer durations for plank, reps for others
  ...
  if (selectEl) {
    if (ex?.isTimerExercise) {
      ...
      selectEl.onchange = () => { state.targetSetReps = parseInt(selectEl.value); };
    }
  }
...
function renderCorrectionPanel(key) {
  ...
  container.innerHTML = `
    <div class="card-title"><i data-lucide="clipboard-check"></i> ${ex.name.toUpperCase()} - FORM GUIDE</div>
    ...
    <div class="steps-list">
      ${ex.steps.map((s, i) => `
        <div class="step-item" onclick="highlightStep(${i})">
          <div class="step-visual"><i data-lucide="${s.icon || 'circle'}"></i></div>
          <div class="step-info">
...
async function sendAiMessage() {
  ...
  // Push user message and show it immediately
  ...
  const exerciseCtx = state.exercise
    ? `User is currently training: ${EXERCISES[state.exercise]?.name}. ` +
      `Current form score: ${state.formScore}%. ` +
      `Muscles targeted: ${EXERCISES[state.exercise]?.musclesWorked?.join(', ')}.`
    : 'No exercise selected yet.';

  const systemPrompt =
    `You are an expert personal trainer and real-time exercise form coach ` +
    `embedded inside an AI Form Coach web app that uses live pose detection.\n` +
    ...
    `${exerciseCtx}\n` +
    `Session stats: ${state.reps} total reps · ${state.sets} sets · ` +
    `${Math.round(state.calories)} kcal burned.\n` +
    `Always prioritise safety and proper form over speed or load.`;
  ...
  try {
    ...
    // Non-2xx means the server sent a structured error
    if (!resp.ok) {
    ...
    // Distinguish offline vs server-down vs other errors
    if (!navigator.onLine) {
      aiSay('⚠️ You appear to be offline. Check your internet connection.');
    } else if (err.message.includes('Failed to fetch') || err.message.includes('NetworkError')) {
      aiSay('⚠️ Cannot reach the server. Is `npm start` running on port 3000?');
    } else {
      aiSay(`⚠️ ${err.message}`);
    }
    ...
    if (input)      input.focus();   // return focus for rapid follow-ups
  }
}
...
function saveSet() {
  const ex = EXERCISES[state.exercise];
  const isTimer = ex?.isTimerExercise;
  const record = {
    date:      new Date().toISOString(),
    exercise:  state.exercise,
    name:      ex?.name,
    set:       state.sets,
    reps:      isTimer ? null : state.targetSetReps,
  ...
  container.innerHTML = state.workoutHistory.slice(0, 50).map(r => {
    const scoreClass = r.formScore >= 80 ? 'good' : r.formScore >= 60 ? 'avg' : 'poor';
      ...
      <div class="history-card">
        <div class="h-ex">${r.name}</div>
        <div class="h-meta">
           ${new Date(r.date).toLocaleDateString()}<br>
          Set ${r.set} · ${r.duration ? `${r.duration}s hold` : `${r.reps} reps`} · ${r.calories} cal
        </div>
        <span class="h-score ${scoreClass}">${r.formScore}% form</span>
      </div>
    `;
  }).join('');
  renderIcons();
