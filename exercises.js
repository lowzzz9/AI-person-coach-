...
const EXERCISES = {
  squat: {
    ...
    steps: [
      { title: "Stance", desc: "Feet shoulder-width apart, toes slightly out", icon: "activity", fix: "Keep feet firmly planted" },
      { title: "Chest Up", desc: "Look forward, not down", icon: "arrow-up", fix: "Avoid rounding your back" },
      { title: "Descend", desc: "Lower hips back and down like sitting in a chair", icon: "move-down", fix: "Knees track over toes" },
      { title: "Depth", desc: "Thighs parallel to floor, 90° angle", icon: "ruler", fix: "Don't let knees cave inward" },
      { title: "Drive Up", desc: "Push through heels explosively", icon: "footprints", fix: "Keep weight in heels" }
    ],
    mistakes: [
    ...
    analyze(keypoints, canvasH) {
      ...
      if (isDown && kneeAngle < 120) {
        ...
        feedback.push("Standing position - begin squatting down.");
        repSignal = false;
      }
  ...
  pushup: {
    ...
    steps: [
      { title: "Plank Position", desc: "Hands shoulder-width apart, wrists under shoulders", icon: "hand", fix: "Wrists directly below shoulders" },
      { title: "Body Line", desc: "Straight line from head to heels", icon: "scan-line", fix: "No sagging or piking hips" },
      { title: "Descend", desc: "Lower chest toward ground, elbows 45°", icon: "arrow-down", fix: "Don't flare elbows wide" },
      { title: "Full Range", desc: "Chest nearly touches ground", icon: "repeat-2", fix: "Control the descent slowly" },
      { title: "Push Up", desc: "Exhale, extend arms fully", icon: "target", fix: "Lock out at top" }
    ],
    mistakes: [
  ...
  lunge: {
    ...
    steps: [
      { title: "Stand Tall", desc: "Feet hip-width, step forward", icon: "lock", fix: "Keep torso upright" },
      { title: "Lower Down", desc: "Front knee bends to 90°", icon: "timer", fix: "Knee stays behind toes" },
      { title: "Back Knee", desc: "Hover back knee 1 inch off floor", icon: "undo-2", fix: "Maintain 90° back knee angle" },
      { title: "Drive Up", desc: "Push through front heel", icon: "trophy", fix: "Keep weight centered" },
      { title: "Alternate", desc: "Switch legs each rep", icon: "user", fix: "Balance equally both sides" }
    ],
    mistakes: [
    ...
    analyze(keypoints) {
      ...
      if (kneeDiff > 60) {
        ...
        score -= 20;
        feedback.push("Lunge deeper - get front knee to 90°.");
  ...
  plank: {
    ...
    ytEmbed: "pSHjTRCQxIw",
    caloriesPerRep: 0.15,
    isTimerExercise: true,       // - hold exercise: use timer, not rep counter
    defaultDuration: 30,         // seconds
    targetAngle: 180,
    musclesWorked: ["Core", "Transverse Abdominis", "Shoulders", "Glutes"],
    steps: [
      { title: "Elbows Under Shoulders", desc: "Forearms flat on ground", icon: "dumbbell", fix: "Stack shoulders directly above elbows" },
      { title: "Straight Line", desc: "Head to heels perfectly aligned", icon: "activity", fix: "No sagging or piking" },
      { title: "Core Braced", desc: "Pull belly button toward spine", icon: "arrow-up", fix: "Brace like absorbing a punch" },
      { title: "Glutes Squeezed", desc: "Full body tension activated", icon: "move-down", fix: "Don't let hips drop" },
      { title: "Breathe", desc: "Steady rhythmic breathing", icon: "ruler", fix: "Don't hold your breath" }
    ],
    mistakes: [
    ...
    analyze(keypoints) {
      ...
      if (lAnkle) {
        ...
        if (deviation < 15) {
          ...
          score = 100;
          repSignal = false; // timer handles completion, not rep signal
        } else if (deviation < 30) {
          score -= 20;
          feedback.push("Slight hip sag - tighten your core!");
  ...
  deadlift: {
    ...
    steps: [
      { title: "Hip Hinge", desc: "Push hips back, maintain neutral spine", icon: "footprints", fix: "Hinge at hips, not waist" },
      { title: "Bar Position", desc: "Bar over mid-foot, arms vertical", icon: "hand", fix: "Keep bar close to shins" },
      { title: "Chest Up", desc: "Proud chest, lats engaged", icon: "scan-line", fix: "No rounded shoulders" },
      { title: "Drive Through Heels", desc: "Push floor away, extend hips", icon: "arrow-down", fix: "Hips and bar rise together" },
      { title: "Lock Out", desc: "Stand tall, hips fully extended", icon: "repeat-2", fix: "Don't hyperextend back" }
    ],
    mistakes: [
    ...
    analyze(keypoints) {
      ...
      if (angle > 160) {
        feedback.push("Standing - hip hinge to begin.");
      } else if (angle < 90) {
        feedback.push("Good hip hinge depth!");
        repSignal = true;
      } else {
        feedback.push("Push hips further back.");
  ...
  bicepCurl: {
    ...
    steps: [
      { title: "Stand Tall", desc: "Elbows tucked to sides", icon: "target", fix: "Don't swing torso" },
      { title: "Curl Up", desc: "Supinate wrists as you curl", icon: "lock", fix: "Full supination at top" },
      { title: "Peak Contraction", desc: "Squeeze bicep hard at top", icon: "timer", fix: "Hold for 1 second" },
      { title: "Slow Down", desc: "3-second controlled descent", icon: "undo-2", fix: "Don't drop weight fast" },
      { title: "Full Extension", desc: "Fully straighten arm at bottom", icon: "trophy", fix: "Full ROM every rep" }
    ],
    mistakes: [
